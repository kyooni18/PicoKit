...
struct Performance {
    ...
    private static func cpuBenchmarks() {
        ...
        var index: UInt32 = 0
        var start = Clock.now()
        while index < iterations { value &+= index; index &+= 1 }
        report("cpu.add", start, value)

        value = 3; index = 0; start = Clock.now()
        while index < iterations { value &*= 1_664_525; value &+= 1_013_904_223; index &+= 1 }
        report("cpu.multiply", start, value)

        value = 0x1357_9bdf; index = 0; start = Clock.now()
        while index < iterations { value = (value << 5) | (value >> 27); index &+= 1 }
        report("cpu.shift", start, value)

        value = 0; index = 0; start = Clock.now()
        while index < iterations { if index & 1 == 0 { value &+= index } else { value &-= index }; index &+= 1 }
        report("cpu.branch", start, value)

        value = 0x2468_ace1; index = 0; start = Clock.now()
        while index < iterations { value ^= value << 13; value ^= value >> 17; value ^= value << 5; index &+= 1 }
        report("cpu.xorshift", start, value)

        value = 65_536; index = 0; start = Clock.now()
        while index < iterations { value = (value &* 65_470) >> 16; index &+= 1 }
        report("cpu.fixedpoint", start, value)

        value = 0; index = 0; start = Clock.now()
        while index < iterations { value &+= (index &* 17) ^ (index >> 3); index &+= 1 }
        report("cpu.mix", start, value)

        value = 0xffff_ffff; index = 0; start = Clock.now()
        while index < iterations { value = (value >> 1) ^ (0xedb8_8320 & (0 &- (value & 1))); index &+= 1 }
    ...
    private static func peripheralBenchmarks() {
        do {
            ...
            while index < iterations { try gpio.toggle(.gpio14); index &+= 1 }
            report("gpio.single_toggle", singleStart, 0)

            index = 0
            let maskStart = Clock.now()
            while index < iterations { try gpio.toggle(mask: mask); index &+= 1 }
            report("gpio.mask_toggle", maskStart, 0)

            let pwm = try PicoPWM(pin: .gpio0, frequency: .kilohertz(1))
            index = 0
            let pwmStart = Clock.now()
            while index < iterations { try pwm.setDutyCycle(UInt16(truncatingIfNeeded: index)); index &+= 1 }
            ...
            while index < iterations { sample = try adc.read(.gpio26); index &+= 1 }
