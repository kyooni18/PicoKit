...
private extension UARTInstance {
    func validate(tx: PicoPin, rx: PicoPin, chip: PicoChip) throws(PicoKitError) {
        guard tx != rx else {
...
public final class PicoUART {
    ...
    public func writeDMA(_ bytes: [UInt8], timeout: Duration) throws(PicoKitError) {
        ...
        #if PICOKIT_PICO_SDK
        let status = bytes.withUnsafeBufferPointer {
            picokit_uart_write_dma_timeout(instance.rawValue, $0.baseAddress, count, timeout.microseconds)
        }
