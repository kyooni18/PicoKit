...
grep -Fq 'picokit_uart_dma_critical_sections[2]' "$bridge"
grep -Fq 'picokit_spi_dma_critical_sections[2]' "$bridge"
grep -Fq 'critical_section_enter_blocking(&picokit_uart_dma_critical_sections[instance])' "$bridge"
grep -Fq 'critical_section_enter_blocking(&picokit_spi_dma_critical_sections[instance])' "$bridge"
grep -Fq 'save_and_disable_interrupts()' "$bridge"
grep -Fq 'gpio_acknowledge_irq(pin, GPIO_IRQ_EDGE_RISE | GPIO_IRQ_EDGE_FALL)' "$bridge"
grep -Fq 'restore_interrupts(interrupt_state)' "$bridge"
