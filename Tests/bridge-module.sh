...
grep -Fq 'int32_t picokit_stdio_write(const char *text)' "$root/Firmware/PicoKitSDKBridge.h"
grep -Fq 'PICOKIT_STDIO_STATUS_DISCONNECTED = -2' "$root/Firmware/PicoKitSDKBridge.h"
grep -Fq 'PICOKIT_STDIO_STATUS_NO_DATA = -3' "$root/Firmware/PicoKitSDKBridge.h"
