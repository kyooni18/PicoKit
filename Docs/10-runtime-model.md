...
is `sh Tests/integration/generated-project.sh`; set `PICO_HARDWARE_TEST=1` only
when a connected board should also be flashed and byte-echo tested.

### Deliberate limits

PicoKit has no async scheduler, DMA, PIO, Wi-Fi/Bluetooth, USB device-class
configuration beyond stdio, multicore coordination, or runtime peripheral
ownership registry. It also leaves I2C repeated starts, SPI mode/chip-select
policy, UART framing/buffering, ADC voltage conversion, and line/protocol
parsing to focused drivers. GPIO interrupt events are bit-coalesced, not
counted.
